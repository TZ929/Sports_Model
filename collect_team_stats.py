#!/usr/bin/env python3
"""
Collect game stats for players from any team that has players in the database.
"""
...
class TeamStatsCollector:
    """Collect game stats for players from a specific team."""
    ...
    def get_team_players(self) -> List[tuple]:
        ...
        with db_manager.get_session() as session:
            ...
            result = session.execute(query, {'team_name': f'%{self.team_name}%'})
            players = result.fetchall()
            
            logger.info(f"Found {len(players)} {self.team_name} players")
            ...
            for row in rows:
                ...
                try:
                    ...
                    game_stat = {
                        ...
                        'player_id': player_id,
                        'team_id': self.team_name[:3].upper(),  # Use first 3 letters as team ID
    ...
    def collect_team_stats(self, save_to_db: bool = True):
        """Collect game stats for all players from the team."""
        
        players = self.get_team_players()
        
        if not players:
            logger.warning(f"No {self.team_name} players found in database")
        ...
        logger.info(f"{self.team_name} collection completed: {successful_players} players, {total_stats} game stats collected")
        return {'players': successful_players, 'stats': total_stats}

def main():
    """Collect game stats for a team with the most players."""
    
    # Let's try the Nets since they have the most players (24)
    team_name = "Nets"
    
    logger.info(f"Starting {team_name} player game stats collection...")
    collector = TeamStatsCollector(team_name)
    results = collector.collect_team_stats(save_to_db=True)
    
    logger.info(f"Collection complete: {results}")
    
    # Show summary
    print(f"\n{'='*50}")
    print(f"{team_name.upper()} COLLECTION SUMMARY")
    print(f"{'='*50}")
